...
<!--            <el-button @click="tagOrCategory('category', article.category.id)" size="mini" type="primary" round plain>{{article.category.categoryName}}</el-button>-->
            ...
            <el-button @click="tagOrCategory('category', category.id)" v-for="category in article.categorys" :key="category.id" size="mini" type="primary" round plain>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      title() {
        return `${this.article.title} - 文章 - 追月亮的猫`
      }
    },
    methods: {
